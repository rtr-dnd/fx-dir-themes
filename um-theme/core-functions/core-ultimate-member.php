...
if ( ! function_exists( 'um_theme_header_friend_request_modal' ) ) {
    function um_theme_header_friend_request_modal() {
        ...
        if ( is_user_logged_in() && class_exists( 'UM' ) && class_exists( 'UM_Friends_API' ) && absint( $defaults['um_show_header_friend_requests'] ) === 1 ) {
...
if ( ! function_exists( 'um_theme_header_activity_modal' ) ) {
    function um_theme_header_activity_modal() {
        ...
        if ( is_user_logged_in() && class_exists( 'UM' ) && class_exists( 'UM_Messaging_API' ) && absint( $defaults['um_show_header_messenger'] ) === 1 ) {
    ...
    <?php
        ...
        foreach ( $notifications as $notification ) {
            if ( ! isset( $notification->id ) ) {
                ...
                $secure_photo_default   = um_secure_media_uri( um_get_default_avatar_uri() );
                $secure_photo           = empty( $notification->photo ) ? $secure_photo_default : um_secure_media_uri( $notification->photo );
            ?>
            ...
            <div class="um-notification <?php echo $notification->type;?> <?php echo $notification->status;?>" data-notification_id="<?php echo $notification->id; ?>" data-notification_uri="<?php echo esc_url( $notification->url ); ?>">
                ...
                <img src="<?php echo esc_url( $secure_photo ); ?>" data-default="<?php echo esc_url( $secure_photo_default ); ?>" class="um-notification-photo">
...
if ( ! function_exists( 'um_theme_header_notification_modal' ) ) {
    function um_theme_header_notification_modal() {
        ...
        if ( is_user_logged_in() && class_exists( 'UM' ) && class_exists( 'UM_Notifications_API' ) && absint( $defaults['um_show_header_notification'] ) === 1 ) {
...
if ( ! function_exists( 'um_theme_friend_box_profile' ) ) {
    function um_theme_friend_box_profile() {
        global $defaults;
        if ( class_exists( 'UM_Friends_API' ) && absint( $defaults['um_show_profile_friend_requests'] ) === 1  ) {
...
if ( function_exists( 'um_messaging_plugins_loaded' ) ) {
    ...
    if ( class_exists( 'UM_Messaging_API' ) && absint( $defaults['um_theme_ext_pm_message_hide_chat_history'] ) === 2 ) {
